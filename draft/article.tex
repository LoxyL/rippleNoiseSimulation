...
粒子辐射探测在空间物理、高能粒子物理及医学成像等领域至关重要\cite{1, 2, 3}。粒子辐射探测器依赖高压开关电源，以扩大耗尽区并在耗尽区内形成强电场，驱动电离产生的电荷载流子(电子-空穴/离子对)向电极迁移，生成可测电信号。
...
\begin{table}[!h]
    ...
    \begin{tabular}{lll}
        ...
        \hline
        $t$ & $2.8\,\mu\mathrm{s}$ & 整形放大电路的时间常数 (Time constant of the shaper) \\
        $R_1$ & $13\,\mathrm{k\Omega}$ & 电源滤波电路电阻 (Power filter resistor) \\
        $C_1$ & $0.1\,\mu\mathrm{F}$ & 电源滤波电路电容 (Power filter capacitor) \\
        $R_2$ & $10\,\mathrm{M\Omega}$ & 电源滤波电路电阻 (Power filter resistor) \\
        $C_2$ & $0.01\,\mu\mathrm{F}$ & 电源滤波电路电容 (Power filter capacitor) \\
        $C_{\mathrm{d}}$ & $2\,\mathrm{pF}$ & 探测器结电容 (Detector junction capacitance) \\
        $C_{\mathrm{x}}$ & $0.1\,\mu\mathrm{F}$ & 隔直电容 (DC-blocking capacitor) \\
        $C_{\mathrm{f}}$ & $3\,\mathrm{pF}$ & 反馈电容 (Feedback capacitor) \\
        $R_{\mathrm{b}}$ & $100\,\mathrm{M\Omega}$ & 偏置电阻 (Bias resistor) \\
        $R_{\mathrm{f}}$ & $200\,\mathrm{M\Omega}$ & 反馈电阻 (Feedback resistor) \\
        $A_0$ & $38 \times e$ & 整形放大电路的增益 (Shaper gain) \\
        \hline
    \end{tabular}
\end{table}
