...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\linewidth]{./non_gaussian_effect.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\linewidth]{./non_gaussian_effect_num.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{./exp_circuit.png}
    \caption{实验用探测器电路}
    \label{fig:exp_circuit}
\end{figure}

实验线路连接示意如图\ref{fig:exp_setup}所示。信号发生器一方面向等效探测器注入 1kHz 方波脉冲用于事件模拟，另一方面在 BBV 处注入 $10\,\mathrm{kHz}$、$5\,\mathrm{V}$ 锯齿波用于模拟纹波。探测器系统的输出信号通过串口传输至 PC，在 PC 端进行数据采集与统计分析，进而获得噪声的 FWHM。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{./exp_setup.png}
    \caption{实验线路连接图}
    \label{fig:exp_setup}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{接入 $70\,\mathrm{kHz}$、$5\,\mathrm{V_{pp}}$ 锯齿纹波的结果}
