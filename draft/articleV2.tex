...
\begin{abstract}
为定量评估高压开关电源纹波对粒子辐射探测器输出噪声的影响，建立基于电荷敏感前置放大器架构的纹波耦合理论模型。推导涵盖电源滤波及信号整形等环节的传递函数，并评估纹波非高斯特性引发的能谱展宽效应。构建全流程电路仿真模型与硬件验证平台，通过向读出电路注入可控锯齿波信号，在宽频带及多幅值条件下进行噪声特性实测。结果表明，理论计算与电路仿真数据在全频段吻合，典型纹波频率区间的误差低于10\%；硬件实测的噪声展宽增量与理论模型预测值的相对偏差约为18\%，验证了所建模型的适用性。研究揭示了高压电源纹波的噪声耦合机制，为粒子辐射探测器系统低噪声高压开关电源的设计与优化提供了理论依据。
\end{abstract}

\noindent\textbf{关键词：}粒子辐射探测器；高压开关电源；电压纹波；噪声分析

\renewcommand{\abstractname}{Abstract}
\begin{abstract}
To quantitatively evaluate the impact of high-voltage switching power supply ripples on the output noise of particle radiation detectors, a theoretical ripple coupling model based on the charge-sensitive preamplifier architecture is established. The transfer function covering the power supply filtering and signal shaping stages is derived, and the energy spectrum broadening effect induced by the non-Gaussian characteristics of the ripples is evaluated. A full-process circuit simulation model and a hardware verification platform are constructed, and noise characteristics are measured under broadband and multi-amplitude conditions by injecting controllable sawtooth wave signals into the readout circuit. The results show that the theoretical calculations are consistent with the circuit simulation data across the entire frequency band, with an error of less than 10\% in the typical ripple frequency range. The relative deviation between the hardware-measured noise broadening increment and the predicted value of the theoretical model is approximately 18\%, verifying the applicability of the established model. The study reveals the noise coupling mechanism of high-voltage power supply ripples, providing a theoretical basis for the design and optimization of low-noise high-voltage switching power supplies for particle radiation detector systems.
\end{abstract}

\noindent\textbf{Keywords: }particle radiation detector; high-voltage switching power supply; voltage ripple; noise analysis

\section{引言}

粒子辐射探测器在空间物理、高能粒子物理及医学成像等领域发挥重要作用\cite{ref1,ref2,ref3}。粒子辐射探测器依赖高压开关电源提供稳定的偏置电压，以扩大耗尽区并在耗尽区内形成强电场，驱动电离产生的电荷载流子(电子-空穴对)向电极迁移，生成可测电信号。

高压开关电源纹波通过多种耦合机制影响探测器的输出信号，导致噪声增加，信噪比降低，从而导致数据失真\cite{ref4}。低纹波的高压开关电源可以确保探测器耗尽区电场随时间的稳定性，使载流子的漂移过程平稳；同时减少耦合到输出端的噪声，提高信噪比及能量分辨率\cite{ref5}。以往关于高压开关电源纹波对粒子辐射探测器系统噪声的影响的研究多停留在定性层面，缺少定量研究\cite{ref6,ref7}。本文将以 Si-PIN 型粒子辐射探测器电路结构为基础，建立从高压开关电源纹波到探测器系统输出噪声的定量理论模型，并通过系统仿真验证其适用性，为提升粒子辐射探测器的测量精度提供理论参考。
...
图\ref{fig:power_filter}所示为电源滤波电路的等效电路，可以看作由两个 RC 低通滤波器级联而成的二阶滤波架构，用于抑制高频噪声\cite{ref8}。高压开关电源的输入电压由 BBV 输入，经过该滤波网络后，通过 H1 端子为探测器提供偏置电压。
...
图\ref{fig:detector}所示为探测器与电荷敏感前置放大器（CSA）的电路结构，探测器可以等效为理想二极管和结电容 $C_{\mathrm{d}}$ 并联。在完全耗尽的情况下，结电容 $C_{\mathrm{d}}$ 可由下式计算\cite{ref9}：
\begin{equation}
C_{\mathrm{d}} = \frac{\varepsilon_0 \varepsilon_{\mathrm{r}} A_{\mathrm{s}}}{D_{\mathrm{d}}}
\end{equation}

其中 $A_{\mathrm{s}}$ 为探测器的等效正对面积(通常在 mm$^2$ 量级)，$D_{\mathrm{d}}$ 为探测器厚度(通常在 $\mu$m 量级)。
...
$C_{\mathrm{x}}$ 为隔直电容，其容值通常在 nF 到 $\mu$F 量级，用于阻隔来自探测器的直流电流。探测器产生的电荷信号经 $C_{\mathrm{x}}$ 耦合至CSA，CSA将其转换为电压信号，并放大至适合后续数字信号处理的电平。

整形放大电路与模数转换器一起内置在 ASIC 中，将放大后的信号整形为类高斯波形，为后续的模数转换做准备。基于实验室现有的基于 RENA-3 读出芯片的探测器板卡，本文假定该电路采用 CR-RC 滤波器的架构，具有成形时间 $\tau$，同时提供增益 $A_0$。
...
如图\ref{fig:ripple}所示，高压开关电源的纹波通常表现为周期性的锯齿波，这主要源于开关器件的脉宽调制（PWM） 控制。其波形的上升沿和下降沿的斜率受开关频率、占空比、前级电压以及控制模式的影响，进而直接决定了纹波幅度及其频谱分布\cite{ref11}。
...
接下来分析偏置电压纹波耦合至 CSA 输出端的频率响应。在 $V_{\mathrm{bias}}$ 处叠加频率为 $f$ 的小幅值交流电压源 $u$，考虑到运算放大器的理想特性，其反相输入端为虚地，故认为该点电势为 0。可求得探测器阴极电压 $u_1$ 为
\begin{equation}
u_1 = u \cdot \frac{j2\pi f C_{\mathrm{d}} R_{\mathrm{b}}}{1 + j2\pi f C_{\mathrm{d}} R_{\mathrm{b}} + j2\pi f C_{\mathrm{x}} R_{\mathrm{x}}}
\end{equation}

CSA 的输出电压 $u_2$ 与探测器阴极电压 $u_{1}$ 的增益由反馈网络阻抗 $Z_{\mathrm{f}}$ 与输入支路阻抗 $Z_{\mathrm{s}}$ 的阻抗比决定，即
\begin{equation}
...
\begin{equation}
u_2 = - \frac{j2\pi f C_{\mathrm{d}} R_{\mathrm{b}}}{(1 + j2\pi f C_{\mathrm{d}} R_{\mathrm{b}} + j2\pi f C_{\mathrm{x}} R_{\mathrm{b}})(j2\pi f C_{\mathrm{f}}(R_{\mathrm{s}} + 1/j2\pi f C_{\mathrm{f}}))}
\end{equation}

从而，从探测器偏置端到 CSA 输出端的纹波耦合传递函数为
\begin{equation}
H_{\mathrm{csa}}(f) = \frac{u_2}{u} = - \frac{j2\pi f C_{\mathrm{d}} R_{\mathrm{b}}}{(1 + j2\pi f C_{\mathrm{d}} R_{\mathrm{b}} + j2\pi f C_{\mathrm{x}} R_{\mathrm{b}})(j2\pi f C_{\mathrm{f}}(R_{\mathrm{s}} + 1/j2\pi f C_{\mathrm{f}}))}
\end{equation}
...
为验证前述理论模型的准确性，本文搭建了基于 Spice 和 Python 的探测器仿真系统，实现了从粒子入射到 ADC 计数的全过程信号模拟，通过测量纹波对计数结果的影响，定量评估高压开关电源纹波对探测器输出噪声的影响。
...
仿真将 Id 设置为幅值 $0.4\,\mathrm{\mu A}$、脉冲宽度 $1\,\mathrm{\mu s}$、周期 $0.987\,\mathrm{ms}$（频率约为 $1013\,\mathrm{Hz}$）的脉冲电流，用以模拟 $9.05\,\mathrm{MeV}$ 的粒子以每秒约 1000 个的通量射入探测器。系统对该电流脉冲的响应为在输出端 Vout 产生一个类高斯形的电压脉冲（如图\ref{fig:circ_sim_response}所示），脉冲高度与粒子能量成正比。
...
为模拟真实探测器的信号处理过程，本文对瞬态仿真输出波形进行分析，设置阈值（1.0V）以提取并统计每个输出脉冲的峰高，并将收集到的所有脉冲峰高数据构建成能谱。电源纹波会导致输出脉冲的基线波动，从而使能谱的峰位展宽。本文以能谱峰的 FWHM 作为指标，定量评估不同参数的纹波对探测器能量分辨率的影响。为更准确地反映真实探测器性能，本文在仿真中引入了额外的高斯噪声，以模拟电子学噪声等系统固有噪声源。此外，由于电源纹波并非高斯白噪声，导致能谱展宽具有非高斯特性，难以通过传统的FWHM进行评估，引入固有高斯噪声不仅能更真实地模拟探测器系统的实际底噪，同时也能将总能谱线型平滑化，使其近似服从高斯分布，便于使用FWHM进行评估。关于纹波非高斯性的详细讨论见 3.3 节。图\ref{fig:circ_sim_fwhm}为纹波幅值 $3.0\,\mathrm{V}$，纹波频率 $10\,\mathrm{kHz}$，固有噪声 $5.00\,\mathrm{keV}$ 的能谱结果，测得总噪声 FWHM 为 $5.56\,\mathrm{keV}$，即纹波引入的噪声为 $\sqrt{5.56^2-5.00^2}\,\mathrm{keV}\approx2.43\,\mathrm{keV}$。
...
变量名 & 值 & 解释 \\
\midrule
$\tau$ & $2.8\,\mathrm{\mu s}$ & 整形放大电路的时间常数 \\
$R_1$ & $13\,\mathrm{k}\Omega$ & 电源滤波电路电阻 \\
$C_1$ & $0.1\,\mathrm{\mu F}$ & 电源滤波电路电容 \\
$R_2$ & $1\,\mathrm{M}\Omega$ & 电源滤波电路电阻 \\
...
$A_0$ & $38 \times \mathrm{e}$ & 整形放大电路的增益 \\
\bottomrule
\end{tabular}
\end{table}
...
本文分别仿真了幅值在 $0.1\,\mathrm{V}$ 至 $10.0\,\mathrm{V}$区间、频率在 $100\,\mathrm{Hz}$ 至 $500\,\mathrm{kHz}$ 区间的纹波对探测器输出 Vout 的影响，每个数据点仿真 3 秒（3000 个峰值），并将仿真结果与理论计算结果进行比较，如图\ref{fig:circ_sim_results}所示。仿真结果在全频段与理论结果吻合。对于高压开关电源纹波的典型频率（60--300 kHz）\cite{ref15}，最高误差不超过 10\%，可知该模型对于高压开关电源纹波的适用性良好。在 10 kHz 的开关频率下，峰峰值为 2V（即幅值为 1V）的纹波在探测器系统输出端引入约 1 keV 的噪声，显著降低系统能量分辨率。
...
\end{equation}
其中，$G(\mu, \sigma^2)$ 表示均值为 $\mu$、方差为 $\sigma^2$ 的高斯概率密度函数。

设输出端固有噪声功率为 $v_\text{other}^2$，两种假设下幅值分布分别为
\begin{equation}
P_\text{total,sine}(x)=P_\text{sine}(x)\ast G(0, v_\text{other}^2)
\end{equation}
和
\begin{equation}
P_\text{total,gaussian}(x)=P_\text{gaussian}(x)\ast G(0, v_\text{other}^2)=G(0, v_\text{other}^2+v_\text{ripple}^2)
\end{equation}
...
本实验所采用的探测器电路结构与第 2 节所述基本相同。为隔离纹波噪声贡献，避免探测器本征噪声（如漏电流噪声、g-r 噪声等）的干扰，本实验以 2pF 电容（等效于面积为 $20\,\mathrm{mm}^2$、厚度为 $100\,\mathrm{\mu m}$ 的探测器）替代探测器元件。实验中由信号发生器产生频率为 $10\,\mathrm{kHz}$、峰峰值为 $10\,\mathrm{V}$ 的锯齿波，并耦合至偏置通道以模拟高压电源纹波；同时在电容两端注入 1kHz 方波脉冲以模拟粒子射入响应。具体结构如图\ref{fig:exp_circuit}所示。电路元件参数详见表 1。
...
无纹波注入、接入纹波注入通路（信号发生器至零）和注入$10\,\mathrm{V_{pp}}$锯齿纹波三种实验工况下的结果分别如表\ref{tab:no_ripple}、表\ref{tab:zero_ripple}、表\ref{tab:saw_ripple}所示。对比表\ref{tab:zero_ripple}和表\ref{tab:saw_ripple}可见，注入锯齿纹波后输出噪声 FWHM 增加约
$\sqrt{5.61^2-4.55^2}\,\mathrm{keV}\approx3.28\,\mathrm{keV}$，与理论结果 $2.78\,\mathrm{keV}$ 吻合较好（相对误差约18\%），验证了本文理论模型在实际系统中的可用性。
...
\midrule
509.0625 & 9380 & 11 & 4.47 \\
610.8750 & 9126 & 11 & 4.47 \\
712.6875 & 8874 & 10 & 4.07 \\
814.5000 & 8624 & 11 & 4.47 \\
916.3125 & 8379 & 11 & 4.47 \\
\midrule
\multicolumn{3}{c}{平均 FWHM / keV} & 4.39 \\
\bottomrule
\end{tabular}
\end{table}
...
信号强度 / keV & Peak / 道数 & FWHM / 道数 & FWHM / keV \\
\midrule
509.0625 & 9408 & 11 & 4.47 \\
610.8750 & 9146 & 11 & 4.47 \\
712.6875 & 8887 & 11 & 4.47 \\
814.5000 & 8632 & 12 & 4.88 \\
916.3125 & 8379 & 11 & 4.47 \\
\midrule
\multicolumn{3}{c}{平均 FWHM / keV} & 4.55 \\
\bottomrule
\end{tabular}
\end{table}
...
信号强度 / keV & Peak / 道数 & FWHM / 道数 & FWHM / keV \\
\midrule
509.0625 & 9422 & 14 & 5.69 \\
610.8750 & 9150 & 14 & 5.69 \\
712.6875 & 8899 & 13 & 5.29 \\
814.5000 & 8670 & 15 & 6.10 \\
916.3125 & 8419 & 13 & 5.29 \\
\midrule
\multicolumn{3}{c}{平均 FWHM / keV} & 5.61 \\
\bottomrule
\end{tabular}
\end{table}
...
本文对高压电源纹波在 Si-PIN 型粒子辐射探测器中引入的输出噪声进行了定量的分析，建立了一个从纹波时域波形预测探测器系统输出噪声 FWHM 的数值模型，并通过实验验证了该模型的适用性。

\begin{graycommentblock}
本文得出的结论为，高压电源纹波是探测器系统输出噪声的重要来源。基于本文验证的线性耦合模型推算，即便是在低纹波高压电源工况下（百毫伏量级），其引入的噪声依然不容忽视；而实验中为显著观测所注入的 $10\,\mathrm{V_{pp}}$ 纹波，会在输出端产生高达数 $\mathrm{keV}$ 的噪声展宽。需要指出的是，虽然本研究基于 Si-PIN 探测器和 RENA-3 读出芯片的特定参数，但所建立的纹波耦合理论模型对其他半导体探测器系统（如 CdTe、CZT 等）同样具有理论指导意义\cite{ref1,ref9}。对于此类采用 CSA 架构的系统，其噪声耦合机制在物理本质上是通用的，仅需根据探测器结电容及电路参数进行重新标定即可应用本模型。然而，对于液体闪烁体探测器等基于光电倍增管（PMT）或硅光电倍增管（SiPM）的系统，由于其信号读出与增益机制的差异，本模型的定量结论不应直接套用。
\end{graycommentblock}

本研究针对高压电源纹波对粒子辐射探测器噪声的影响进行了深入的定量剖析，补充了现有研究在这一细分领域的理论模型与实验验证。本文所提出的理论模型与实验方法可为高性能粒子探测系统的低噪声电源设计提供重要的理论依据和设计参考。
