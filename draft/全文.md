...
整形放大电路通常与数模转换器一起内置在ASIC中，将放大后的信号整形为类高斯波形，为后续的模数转换做准备。本文假定该电路采用CR-RC滤波器的架构，具有成形时间τ，同时提供增益$A_0$。

> 需要指出的是，实际探测器系统中的互连线存在寄生电容、电感和电阻。根据相关噪声模型分析[13]，在本文研究的纹波频率范围（60 kHz–300 kHz）内，寄生电感和电阻引起的阻抗通常微不足道。对于寄生电容而言，虽然前置放大器的噪声特性对输入端总电容敏感，但在探测器结电容 $C_{\mathrm{d}}$（pF 量级）占主导地位且系统无高频谐振的前提下，互连线寄生电容对传递函数幅频特性的影响相对有限。此外，基于系统在工作点附近满足线性时不变（LTI）的假设，且截止频率远低于纹波高次谐波频率，寄生参数带来的非线性及高频效应可被有效过滤。因此，在本模型的推导中，互连线的寄生电容及其他寄生参数将被忽略，以简化分析过程。
...
> 为说明将锯齿纹波等效为同平均功率正弦波的合理性，我们在“小信号加性扰动、系统在工作点附近线性时不变（LTI）”的前提下，从功率传输角度论证如下。

> 设输入扰动为 x(t)，系统传递函数为 H(f)。根据 Parseval 定理与功率传输关系，输出均方值为
\[
\sigma_y^2=\int_{-\infty}^{\infty}\lvert H(f)\rvert^2\,S_x(f)\,df,
\]
其中 \(S_x(f)\) 为 x(t) 的功率谱密度。锯齿纹波可分解为基波及其谐波的离散谱线，且总功率
\[
P_{\mathrm{ripple}}=\frac{V_{\mathrm{ripple}}^2}{12}.
\]
若以同平均功率的正弦波替代之（本工作已给出 \(V_{\mathrm{pp,sin}}=\sqrt{2/3}\,V_{\mathrm{ripple}}\)），则其谱能量集中于 \(f_0\) 处，且总输入功率保持不变：
\[
P_{\mathrm{sin}}=\frac{V_{\mathrm{pp,sin}}^2}{8}=P_{\mathrm{ripple}}.
\]
当系统 \(\lvert H(f)\rvert\) 在锯齿主要谐波所覆盖的频带内近似平坦，或系统截止频率低于 \(2f_0\) 仅保留基波（常见于前端低通/积分型读出），则有
\[
\sigma_{y,\mathrm{sin}}^2\approx \sigma_{y,\mathrm{ripple}}^2,
\]
即以同功率正弦代替锯齿对输出方差/等效噪声功率的影响等价。更进一步，若 \(\lvert H(f)\rvert\) 随频率衰减（低通），将多谐波功率“集中”到基波的等功率正弦会给出略大的输出功率估计，因而是保守近似。

> 从非线性小信号展开的角度，设输出指标与输入扰动的关系在工作点处可二阶展开：
\[
g(x_0+\Delta v)\approx g(x_0)+g'(x_0)\Delta v+\tfrac12 g''(x_0)\Delta v^2.
\]
对零均值纹波 \(\mathbb{E}[\Delta v]=0\)，主要影响项为 \(\tfrac12 g''(x_0)\mathbb{E}[\Delta v^2]\)，即只与扰动的方差（平均功率）有关，同样支持功率等效。

> 需要说明的是，若系统对高次谐波具选择性增益（\(\lvert H(f)\rvert\) 随频率显著上升），或指标受峰值/占空比/相位锁定等形状特征主导（如阈值触发、饱和），则应改用频域加权求和
\(\sigma_y^2=\sum_n \lvert H(nf_0)\rvert^2 P_n\)，或直接以基波等效（\(V_{\mathrm{pp,\,fund}}=\tfrac{2}{\pi}V_{\mathrm{ripple}}\)）进行对比。由于
\(\sqrt{2/3}\approx 0.816>\tfrac{2}{\pi}\approx 0.637\)，在仅基波通过的场景中，本文采用的等功率正弦幅值更大，因此给出的是保守估计。
...
换算成以 eV 为单位的半峰全宽（Full Width at Half Maximum, FWHM）[14]得

$$
\mathrm{FWHM} = 2.35\times3.62\times \frac{\mathrm{ENC}}{q} = 8.5 \cdot \frac{e\cdot v_{\mathrm{ripple,out}} \cdot C_{\mathrm{f}}}{A_0q}
$$

> 其中 $e$ 为自然常数，$C_{\mathrm{f}}$ 为反馈电容值，$q$ 为元电荷量。$\mathrm{ENC}$ 为噪声等效电荷量，可表示为
\[
\mathrm{ENC} = v_{\mathrm{ripple,in}} \cdot C_{\mathrm{d}} = v_{\mathrm{ripple,out}} \cdot \frac{e}{A_0} \cdot C_{\mathrm{f}}
\]
这里乘了个e，是考虑到一阶CR-RC整形电路输出信号的峰值存在1/e的衰减。
将ENC转化为噪声等效能量\[E_{\mathrm{ENC}} = 3.62\times\frac{\mathrm{ENC}}{q}\quad (\mathrm{eV})\]
其中3.62为单个电子-空穴对等效的能量。
高斯分布中，FWHM与标准差的关系为\[FWHM = 2.355\times\sigma\]，因此\[FWHM = 2.355\times\frac{E_{\mathrm{ENC}}}{q}\]
...
![图10 纹波耦合到输出端的FWHM差异量化](./non_gausian_effect_num.png)

## 4 实验验证

在仿真分析的基础上，本文进一步使用实际高压电源及 Si-PIN 型粒子辐射探测器电路，实测高压电源纹波对探测器输出噪声的影响，以验证理论模型的适用性。

本实验所采用的探测器电路结构与第 2 节所述基本相同。为隔离出高压电源纹波的噪声贡献，避免探测器自身的噪声（如漏电流噪声、g-r 噪声等）的干扰，本实验以 2pF 电容（相当于一个面积为 $20\mathrm{mm}^2$，厚度为 $100\mathrm{\mu m}$ 的探测器）替代探测器元件，并在电容两端注入方波信号，以模拟粒子射入。具体结构如图 12 所示。电路元件参数详见表 1。

![图12 实验用探测器电路](./exp_circuit.png)

实验线路连接示意如图 13 所示。信号发生器与 2pF 电容（等效探测器）两端相连。探测器系统的输出信号传输至 PC，在 PC 端进行数据采集与统计分析，进而获得噪声的 FWHM 数据。

![图13 实验线路连接图](./exp_setup.png)

在本实验中，信号发生器产生了一系列不同幅度的 1kHz 方波信号，以模拟每秒 1000 个粒子射入探测器。通过调节信号发生器的输出幅度，实现了不同强度信号的输入，从而模拟探测器对不同能量粒子事件的响应，并测量相应的 FWHM。

不接高压电源、接 0V 高压电源、接 -200V 高压电源实验结果分别如表 2、表 3、表 4 所示。对比表 3 和表 4，可知 -200V 的高压电源引入了约 1keV 的噪声，与数值仿真的结果（约 1keV）一致，验证了本文所建立的从高压电源纹波到探测器输出噪声的理论模型的准确性。
